...
def test_basic_route():
    ...
    assert report["safer_route"]["path"][0] == "L1"
    assert report["safer_route"]["path"][-1] == "L5"
    print("PASS: basic route L1 -> L5 found")


def test_avoids_critical_roads():
    report = build_routing_report(start_id="L1", end_id="L5")
    roads_used = set(report["safer_route"]["roads_used"])
    # R1 (L1-L2) and R4 (L2-L4) are Critical; R2/R5/R12/R8 route around them
...
def test_invalid_location():
    report = build_routing_report(start_id="L1", end_id="L99")
